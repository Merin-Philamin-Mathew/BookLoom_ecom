...
def cart(request, total=0, quantity=0, cart_items = None):
    try:
        ...
        if request.user.is_authenticated:
            cart_items = CartItem.objects.filter(user=request.user, is_stock=True).order_by('-id')
           
        else:
            cart = Cart.objects.get(cart_id = _cart_id(request))
            cart_items = CartItem.objects.filter(cart=cart, is_stock=True).order_by('-id')
    ...
    if current_user.is_authenticated:
        try:
            ...
            if cart_item.product.stock < cart_item.quantity:
                ...
                messages.warning(request, f'Sorry, {cart_item.product.product.product_name} stock limit exceeded.')
